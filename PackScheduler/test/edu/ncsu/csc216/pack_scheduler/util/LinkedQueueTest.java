...
public class LinkedQueueTest {
	...
	public void testLinkedQueue() {
		...
		try {
			q.enqueue("kiwi");
			fail("LinkedQueue should have default capacity of 10, so adding an 11th element should be invalid");
		} catch (IllegalArgumentException e) {
			assertEquals("LinkedQueue is full", e.getMessage());
	...
	public void testEnqueue() {
		...
		try {
			q.enqueue(null);
			fail("Adding null to LinkedQueue should be invalid");
		} catch (IllegalArgumentException e) {
		...
		try {
			q.enqueue(s);
			fail("Adding a null element to LinkedQueue should be invalid");
		} catch (IllegalArgumentException e) {
			assertEquals("Cannot add null value", e.getMessage());
			assertEquals(0, q.size());
		}
	...
	public void testDequeue() {
		...
		try {
			q.dequeue();
			fail("Removing element from an empty LinkedQueue should be invalid");
		} catch (NoSuchElementException e) {
			assertEquals("LinkedQueue is empty", e.getMessage());
		}
